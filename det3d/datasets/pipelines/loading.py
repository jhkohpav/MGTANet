...
class LoadPointCloudFromFile(object):
    ...
    def __call__(self, res, info):
        ...
        if self.type == "NuScenesDataset":
        ...
        elif self.type == "NuScenesDatasetVID":

            # if self.sequence_mode == 'online':
            #     cur_idx = 0
            ...
            for seq in range(self.seq_len):
                lidar_path = Path(info["lidar_path"][seq])
                points = read_file(str(lidar_path), painted=res["painted"])
                ...
                assert (nsweeps - 1) == len(
                    ...
                    nsweeps, len(info["sweeps"][seq])
                )

                time_lag_list = np.random.choice(len(info["sweeps"][seq]), nsweeps - 1, replace=False)
                # time_lag_list = [0,1,2,3,4,5,6,7,8]
                
                for i in time_lag_list:
                    sweep = info["sweeps"][seq][i]
                    points_sweep, times_sweep = read_sweep(sweep, painted=res["painted"])
                    sweep_points_list.append(points_sweep)
                    sweep_times_list.append(times_sweep)
                ...
                res["lidar"]["points"].append(points)
                res["lidar"]["times"].append(times)
                res["lidar"]["combined"].append(np.hstack([points, times]))
...
class LoadPointCloudAnnotations(object):
    ...
    def __call__(self, res, info):
        ...
        # import pdb; pdb.set_trace()
        if res["type"] in ["NuScenesDataset", "NuScenesDatasetVID"] and "gt_boxes" in info:
            if res['type'] == "NuScenesDataset":
                ...
                for seq in range(self.seq_len):
                    if seq == 0:
                        gt_boxes = info["gt_boxes"][seq].astype(np.float32)
                        gt_boxes[np.isnan(gt_boxes)] = 0
                        res["lidar"]["annotations"] = [{
                            "boxes": gt_boxes,
                            "names": info["gt_names"][seq],
                            "tokens": info["gt_boxes_token"][seq],
                            "velocities": info["gt_boxes_velocity"][seq].astype(np.float32),
                        }]
                    else:
                        gt_boxes = info["gt_boxes"][seq].astype(np.float32)
                        gt_boxes[np.isnan(gt_boxes)] = 0
                        res["lidar"]["annotations"].append({
                            "boxes": gt_boxes,
                            "names": info["gt_names"][seq],
                            "tokens": info["gt_boxes_token"][seq],
                            "velocities": info["gt_boxes_velocity"][seq].astype(np.float32),
