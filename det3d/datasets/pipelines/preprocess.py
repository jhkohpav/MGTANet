...
from det3d.builder import build_dbsampler, build_dbsampler_vid, build_dbsampler_vid_temporal_sampling
...
def _dict_select_vid(dict_, inds, seq_len):
    for k, v in dict_.items():
        for seq in range(seq_len):
            if isinstance(v[seq], dict):
                _dict_select(v[seq], inds[seq])
            else:
                if seq == 0:
                    dict_[k] = [v[seq][inds[seq]]]
                else:
                    dict_[k].append(v[seq][inds[seq]])
...
@PIPELINES.register_module
class PreprocessVID(object):
    def __init__(self, cfg=None, **kwargs):
        self.shuffle_points = cfg.shuffle_points
        self.min_points_in_gt = cfg.get("min_points_in_gt", -1)
        self.seq_len = kwargs.get('seq_len')
        self.sequence_mode = kwargs.get('sequence_mode')
        self.gt_sample_mode = kwargs.get('gt_sample_mode')
        ...
        if self.mode == "train":
            ...
            if cfg.db_sampler != None:
                if self.seq_len > 1:
                    if self.gt_sample_mode == 'temporal':
                        self.db_sampler = build_dbsampler_vid_temporal_sampling(cfg.db_sampler)
                    else:
                        self.db_sampler = build_dbsampler_vid(cfg.db_sampler)
                else:
                    self.db_sampler = build_dbsampler(cfg.db_sampler)
            else:
                self.db_sampler = None 
    ...
    def __call__(self, res, info):
        ...
        if res["type"] in ["WaymoDataset"]:
            if "combined" in res["lidar"]:
                ...
                points = res["lidar"]["points"]
        elif res["type"] in ["NuScenesDataset", "NuScenesDatasetVID"]:
            points = res["lidar"]["combined"]
        else:
            raise NotImplementedError

        if res["type"] == "NuScenesDataset" or res['type'] == "WaymoDataset":
            if self.mode == "train":
                anno_dict = res["lidar"]["annotations"]
            ...
            if self.mode == "train":
                res["lidar"]["annotations"] = gt_dict
        elif res['type'] == "NuScenesDatasetVID":
            # import pdb; pdb.set_trace()
            if self.mode == "train":
                anno_dict = res["lidar"]["annotations"]
                ...
                for seq in range(self.seq_len):
                    if seq == 0:
                        if not anno_dict[seq] is None:
                            gt_dict = {
                                "gt_boxes": [anno_dict[seq]["boxes"]],
                                "gt_names": [np.array(anno_dict[seq]["names"]).reshape(-1)],
                            }
                        else:
                            gt_dict = {
                                "gt_boxes": [np.zeros((1, 9))],
                        ...
                        if not anno_dict[seq] is None:
                            gt_dict['gt_boxes'].append(anno_dict[seq]["boxes"])
                            gt_dict['gt_names'].append(np.array(anno_dict[seq]["names"]).reshape(-1))
                        else:
                            gt_dict['gt_boxes'].append(np.zeros((1, 9)))
                            gt_dict['gt_names'].append(np.array(['car']).reshape(-1))
            ...
            if self.mode == "train" and len(self.augmentation) > 0:
                ...
                selected = [drop_arrays_by_name(
                    gt_dict["gt_names"][seq], ["DontCare", "ignore", "UNKNOWN"]
                ) for seq in range(self.seq_len)]

                _dict_select_vid(gt_dict, selected, self.seq_len)
                ...
                gt_boxes_mask = [np.array(
                    [n in self.class_names for n in gt_dict["gt_names"][seq]], dtype=np.bool_
                ) for seq in range(self.seq_len)]
                ...
                if self.db_sampler and 'db_sample' in self.augmentation:
                    ...
                    if self.gt_sample_mode == 'original':
                        sampled_dict = self.db_sampler.sample_all_vid(
                            res["metadata"]["image_prefix"],
                            gt_dict["gt_boxes"],
                            gt_dict["gt_names"],
                            res["metadata"]["num_point_features"],
                    ...
                    elif self.gt_sample_mode == 'temporal':
                        sampled_dict = self.db_sampler.temporal_sample_all_vid(
                            self.sequence_mode,
                            res["metadata"]["image_prefix"],
                            gt_dict["gt_boxes"],
                            gt_dict["gt_names"],
                    ...
                    if sampled_dict is not None:
                        if type(sampled_dict['gt_names']) == np.ndarray:
                            sampled_gt_names = sampled_dict["gt_names"]
                            sampled_gt_boxes = sampled_dict["gt_boxes"]
                            sampled_points = sampled_dict["points"]
                            sampled_points_idx = sampled_dict["points_idx"]
                            sampled_gt_masks = sampled_dict["gt_masks"]
                            raw_gt_box_num = len(gt_dict["gt_boxes"])
                            raw_point_idx = [-1 * np.ones(len(points[i]), dtype=np.int64) for i in range(len(points))]
                            gt_dict["gt_names"] = [np.concatenate(
                                [gt_dict["gt_names"][i], sampled_gt_names], axis=0
                            ) for i in range(len(gt_dict["gt_names"]))]
                            gt_dict["gt_boxes"] = [np.concatenate(
                                [gt_dict["gt_boxes"][i], sampled_gt_boxes]
                            ) for i in range(len(gt_dict["gt_boxes"]))]
                            gt_boxes_mask = [np.concatenate(
                                [gt_boxes_mask[i], sampled_gt_masks], axis=0
                            ) for i in range(len(gt_boxes_mask))]

                            # points = np.concatenate([sampled_points, points], axis=0)
                            points = [np.concatenate([points[i], sampled_points], axis=0) for i in range(len(points))]
                            points_idx = [np.concatenate([raw_point_idx[i], sampled_points_idx], axis=0) for i in range(len(raw_point_idx))]
                        
                        elif type(sampled_dict['gt_names']) == list:
                            sampled_gt_names = sampled_dict["gt_names"]
                            sampled_gt_boxes = sampled_dict["gt_boxes"]
                            sampled_points = sampled_dict["points"]
                            sampled_points_idx = sampled_dict["points_idx"]
                            ...
                            gt_names_update_list = []
                            gt_boxes_update_list = []
                            gt_boxes_mask_update_list = []
                            points_idx = []
                            for i in range(self.seq_len):
                                gt_names_update_list.append(np.concatenate([gt_dict["gt_names"][i], sampled_gt_names[i]], axis=0))
                                gt_boxes_update_list.append(np.concatenate([gt_dict["gt_boxes"][i], sampled_gt_boxes[i]]))
                                gt_boxes_mask_update_list.append(np.concatenate([gt_boxes_mask[i], sampled_gt_masks[i]], axis=0))

                                points[i] = np.concatenate([points[i], sampled_points[i]], axis=0)
                                points_idx.append(np.concatenate([raw_point_idx[i], sampled_points_idx[i]], axis=0))
                                
                            gt_dict["gt_names"] = gt_names_update_list
                            gt_dict["gt_boxes"] = gt_boxes_update_list
                            gt_boxes_mask       = gt_boxes_mask_update_list
                ...
                _dict_select_vid(gt_dict, gt_boxes_mask, self.seq_len)

                gt_classes = [np.array(
                    [self.class_names.index(n) + 1 for n in gt_dict["gt_names"][i]],
                    dtype=np.int32,
                )for i in range(len(gt_dict["gt_names"]))]
                gt_dict["gt_classes"] = gt_classes
                ...
                res['aug_matrix_inv'] = {}
                if 'flip' in self.augmentation:
                    gt_dict["gt_boxes"], points, enable_xy = prep.random_flip_both_vid(gt_dict["gt_boxes"], points, return_enable_xy=True)
                    flip_mat_T_inv = np.array(
                        [[[1, -1][enable_xy[1]], 0, 0], [0, [1, -1][enable_xy[0]], 0], [0, 0, 1]],
                ...
                if 'rotate' in self.augmentation:
                    gt_dict["gt_boxes"], points, noise_rotation = prep.global_rotation_vid(
                        gt_dict["gt_boxes"], points, rotation=self.global_rotation_noise, return_rot_noise=True
                    )
                    rot_sin = np.sin(noise_rotation*-1)
                    rot_cos = np.cos(noise_rotation*-1)
                ...
                if 'rescale' in self.augmentation:
                    ...
                    gt_dict["gt_boxes"], points, scale_noise = prep.global_scaling_v2_vid(
                        gt_dict["gt_boxes"], points, *self.global_scaling_noise, return_scale_noise=True
                    )
                    scale_mat_T_inv = np.array(
                        [[1/scale_noise, 0, 0], [0, 1/scale_noise, 0], [0, 0, 1/scale_noise]],
                ...
                if 'translate' in self.augmentation:
                    gt_dict["gt_boxes"], points, translate_mat_T = prep.global_translate_vid_(
                        gt_dict["gt_boxes"], points, noise_translate_std=self.global_translate_std, return_noise_T=True
                    )
                    translate_mat_T_inv = translate_mat_T * -1
                    res['aug_matrix_inv']['translate'] = translate_mat_T_inv
    ...
    def deb_video_data(self, points_all, gt_dict_all):
        import os
        
        gt_boxes_all = gt_dict_all['gt_boxes']
        gt_classes_all = gt_dict_all['gt_classes']
        for seq, (points, gt_boxes, gt_classes) in enumerate(zip(points_all, gt_boxes_all, gt_classes_all)):
            np.save(os.getcwd() + '/deb_data/pcd_numpy/' + '%06d'%seq, points)
            np.save(os.getcwd() + '/deb_data/corners_numpy/' + '%06d'%seq, gt_boxes)
            np.save(os.getcwd() + '/deb_data/class_label_numpy/' + '%06d'%seq, gt_classes)
        import pdb; pdb.set_trace()
...
class VoxelizationVID(object):
    ...
    def __call__(self, res, info):
        ...
        if res["mode"] == "train":
            gt_dict = res["lidar"]["annotations"]
            bv_range = pc_range[[0, 1, 3, 4]]
            mask = [prep.filter_gt_box_outside_range(gt_boxes, bv_range) for gt_boxes in gt_dict["gt_boxes"]]
            _dict_select_vid(gt_dict, mask, self.seq_len)
        ...
        for seq_idx in range(self.seq_len):
            voxels, coordinates, num_points = self.voxel_generator.generate(
                res["lidar"]["points"][seq_idx], max_voxels=max_voxels 
            )
            num_voxels = np.array([voxels.shape[0]], dtype=np.int64)
...
class AssignLabelVID(object):
    ...
    def __call__(self, res, info):
        # if self.sequence_mode == 'copy' or self.sequence_mode == 'online':
        #     cur_idx = 0
        ...
        if res["mode"] == "train":
            ...
            for class_name in class_names_by_task:
                task_masks.append(
                    [
                        np.where(
                            gt_dict["gt_classes"][0] == class_name.index(i) + 1 + flag
                        )
                        for i in class_name
                    ]
                )
            ...
            for idx, mask in enumerate(task_masks):
                ...
                task_name = []
                for m in mask:
                    task_box.append(gt_dict["gt_boxes"][0][m])
                    task_class.append(gt_dict["gt_classes"][0][m] - flag2)
                    task_name.append(gt_dict["gt_names"][0][m])
                task_boxes.append(np.concatenate(task_box, axis=0))
                task_classes.append(np.concatenate(task_class))
                task_names.append(np.concatenate(task_name))
                flag2 += len(mask)
            ...
            for idx, task in enumerate(self.tasks):
                ...
                # import pdb; pdb.set_trace()
                if res['type'] == 'NuScenesDataset' or res['type'] == 'NuScenesDatasetVID':
                    # [reg, hei, dim, vx, vy, rots, rotc]
                    anno_box = np.zeros((max_objs, 10), dtype=np.float32)
                elif res['type'] == 'WaymoDataset':
                    anno_box = np.zeros((max_objs, 10), dtype=np.float32) 
