...
def _lidar_nusc_box_to_global(nusc, boxes, sample_token):
    try:
        if isinstance(sample_token, list):
            s_record = nusc.get("sample", sample_token[0])
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, nsweeps=10, filter_zero=True, with_image=False):
    from nuscenes.utils.geometry_utils import transform_matrix
...
def create_nuscenes_infos(root_path, version="v1.0-trainval", nsweeps=10, filter_zero=True, with_image=False):
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, train_scenes, val_scenes, test, nsweeps=nsweeps, filter_zero=filter_zero, with_image=with_image
    )
