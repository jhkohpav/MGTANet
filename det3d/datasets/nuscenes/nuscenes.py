...
class NuScenesDataset(PointCloudDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.with_info:
            self.data_info = NuScenes(version=version, dataroot=root_path, verbose=True)
    ...
    def load_infos(self, info_path):
        print('self._info_path', self._info_path)
    ...
    def get_sensor_data(self, idx):
        ...
        if self.with_info:
            res["data_info"] = self.data_info

        data, _ = self.pipeline(res, info)
        if self.with_info:
            res.pop("data_info")

        if 'aug_matrix_inv' in res:
            if isinstance(data, list):
                for i in range(len(data)):
                    data[i]['aug_matrix_inv'] = res['aug_matrix_inv']
            else:
                data['aug_matrix_inv'] = res['aug_matrix_inv']
...
@DATASETS.register_module
class NuScenesDatasetVID(PointCloudDataset):
    NumPointFeatures = 5  # x, y, z, intensity, ring_index
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(NuScenesDatasetVID, self).__init__(
            root_path, info_path, pipeline, test_mode=test_mode, class_names=class_names
        )
        ...
        self.with_info = kwargs.get('with_info', False)
        self._num_point_features = NuScenesDatasetVID.NumPointFeatures
        self._name_mapping = general_to_detection
    ...
    def evaluation(self, detections, output_dir=None, testset=False):
        ...
        for det in dets:
            annos = []
            boxes = _second_det_to_nusc_box(det)
            boxes = _lidar_nusc_box_to_global(nusc, boxes, det["metadata"]["token"][0])
            for i, box in enumerate(boxes):
                name = mapped_class_names[box.label]
                if np.sqrt(box.velocity[0] ** 2 + box.velocity[1] ** 2) > 0.2:
                    if name in [
                ...
                nusc_anno = {
                    "sample_token": det["metadata"]["token"][0],
                    "translation": box.center.tolist(),
                    "size": box.wlh.tolist(),
                    "rotation": box.orientation.elements.tolist(),
                    "velocity": box.velocity[:2].tolist(),
                ...
                annos.append(nusc_anno)
            nusc_annos["results"].update({det["metadata"]["token"][0]: annos})
