...
class DBFilterByDifficulty_temporal_filtering(DataBasePreprocessing):
    def __init__(self, removed_difficulties, logger=None):
        self._removed_difficulties = removed_difficulties
        logger.info(f"{removed_difficulties}")

    def _preprocess(self, db_infos_list:list):
        new_db_infos_list = [{} for t in range(len(db_infos_list))]

        for key, _ in db_infos_list[0].items():
            init_mask = np.ones(shape=len(db_infos_list[0][key]))
            for t in range(len(db_infos_list)):
                temp_mask = np.array([info["difficulty"] not in self._removed_difficulties for info in db_infos_list[t][key]])
                init_mask *= temp_mask
            init_mask = init_mask.astype(int)
            
            for t in range(len(db_infos_list)):
                new_db_infos_list[t][key] = [info for idx, info in enumerate(db_infos_list[t][key]) if init_mask[idx] == 1]
        
        return new_db_infos_list

class DBFilterByMinNumPoint_temporal_sampling(DataBasePreprocessing):
    ...
    def _preprocess(self, db_infos_list:list):
        ...
        for name, min_num in self._min_gt_point_dict.items():
            init_mask = np.ones(shape=len(db_infos_list[0][name]))
            if min_num > 0:
                for t in range(len(db_infos_list)):
                    temp_mask = np.array([info["num_points_in_gt"] >= min_num for info in db_infos_list[t][name]])
                    init_mask *= temp_mask
            
                init_mask = init_mask.astype(int)
                for t in range(len(db_infos_list)):
                    new_db_infos_list[t][name] = [info for idx, info in enumerate(db_infos_list[t][name]) if init_mask[idx] == 1]
...
def global_rotation(gt_boxes, points, rotation=np.pi / 4, return_rot_noise=False):
...
def global_rotation_vid(gt_boxes_all, points_all, rotation=np.pi / 4,
                        return_rot_noise=False):
    if not isinstance(rotation, list):
        rotation = [-rotation, rotation]
    noise_rotation = np.random.uniform(rotation[0], rotation[1])
    # import pdb; pdb.set_trace()
    for seq_idx, (gt_boxes, points) in enumerate(zip(gt_boxes_all, points_all)):
        points[:, :3] = box_np_ops.rotation_points_single_angle(
            points[:, :3], noise_rotation, axis=2
        )
        gt_boxes[:, :3] = box_np_ops.rotation_points_single_angle(
        ...
        gt_boxes_all[seq_idx] = gt_boxes
        points_all[seq_idx] = points

    if return_rot_noise:
        return gt_boxes_all, points_all, noise_rotation
    return gt_boxes_all, points_all
...
def random_flip_both(gt_boxes, points, probability=0.5, flip_coor=None, return_enable_xy=False):
    # x flip 
    enable_x = np.random.choice(
        [False, True], replace=False, p=[1 - probability, probability]
    )
    if enable_x:
        gt_boxes[:, 1] = -gt_boxes[:, 1]
    ...
    if return_enable_xy:
        return gt_boxes, points, [int(enable_x), int(enable_y)]

    return gt_boxes, points


def random_flip_both_vid(gt_boxes_all, points_all, probability=0.5, flip_coor=None, return_enable_xy=False):
    # import pdb; pdb.set_trace()
    # x flip 
    enable_x = np.random.choice(
        [False, True], replace=False, p=[1 - probability, probability]
    ...
    if return_enable_xy:
        return gt_boxes_all, points_all, [int(enable_x), int(enable_y)]

    return gt_boxes_all, points_all

def global_scaling_v2(gt_boxes, points, min_scale=0.95, max_scale=1.05, return_scale_noise=False):
...
def global_scaling_v2_vid(gt_boxes_all, points_all, min_scale=0.95, max_scale=1.05,
                          return_scale_noise=False):
    # import pdb; pdb.set_trace()
    noise_scale = np.random.uniform(min_scale, max_scale)
    for seq_idx, (gt_boxes, points) in enumerate(zip(gt_boxes_all, points_all)):
        points[:, :3] *= noise_scale
    ...
    if return_scale_noise:
        return gt_boxes_all, points_all, noise_scale
    return gt_boxes_all, points_all
...
def global_translate_(gt_boxes, points, noise_translate_std, return_noise_T=False):
    """
...
def global_translate_vid_(gt_boxes_all, points_all, noise_translate_std,
                          return_noise_T=False):
    """
    Apply global translation to gt_boxes and points.
    """
    if not isinstance(noise_translate_std, (list, tuple, np.ndarray)):
    ...
    if return_noise_T:
        return gt_boxes_all, points_all, noise_translate
