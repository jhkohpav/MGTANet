...
from .encoder_temporal import SeqDeformableEncoderLayer, SeqDeformableEncoder
...
class STFA(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        encoder_layer = SeqDeformableEncoderLayer(d_model = self.src_input_channels,
                                               cfg = self.encoder_cfg, feat_h=feat_h, 
                                               feat_w=feat_w)
        self.encoder = SeqDeformableEncoder(encoder_layer, self.encoder_cfg.num_layers)
        ...
        logger.info("Finish Spatio-Temporal Feature Aggregation Initialization")
    ...
    def forward(self, aligned_x_all):
        ...
        if self.with_pos_emb:
            memory = self.encoder(src_list, spatial_shapes, valid_ratio, 
                                  level_start_index, pos_emb_list)
        else:
            memory = self.encoder(src_list, spatial_shapes, valid_ratio, 
                                  level_start_index)
