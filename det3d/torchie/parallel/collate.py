...
def collate_kitti(batch_list, samples_per_gpu=1):
    ...
    for key, elems in example_merged.items():
        if key in ["voxels", "num_points", "num_gt", "voxel_labels", "num_voxels",
            ...
            ret[key] = elems
        elif key in ["calib", "cam", "image_shape"]:
            ret[key] = {}
            for elem in elems:
...
def collate_kitti_vid(batch_list, samples_per_gpu=1):
    example_merged = collections.defaultdict(list)
    ...
    # import pdb; pdb.set_trace()
    batch_size = len(example_merged['metadata'])
    seq_len = len(batch_list[0]['voxels'])
    ret = {}
    # voxel_nums_list = example_merged["num_voxels"]
    # example_merged.pop("num_voxels")
    for key, elems in example_merged.items():
        if key in ["voxels", "num_points", "num_gt", "voxel_labels", "num_voxels",
                   "cyv_voxels", "cyv_num_points", "cyv_num_voxels"]:
            # import pdb; pdb.set_trace()
            ret[key] = [torch.tensor(np.concatenate([elems[i][seq] for i in range(batch_size)], axis=0)) for seq in range(seq_len)]
        elif key in [
            "gt_boxes",
        ]:
            # import pdb; pdb.set_trace()
            ...
            for i, coor in enumerate(elems):
                coor_pad = [np.pad(
                    coor[seq], ((0, 0), (1, 0)), mode="constant", constant_values=i
                ) for seq in range(seq_len)]
                coors.append(coor_pad)

            ret[key] = [torch.tensor(np.concatenate([coors[i][seq] for i in range(batch_size)], axis=0)) for seq in range(seq_len)]
        elif key in ["anchors", "anchors_mask", "reg_targets", "reg_weights", "labels", "hm", "anno_box",
                    "ind", "mask", "cat"]:
