...
from det3d.datasets import DATASETS, build_dataloader, build_dataloader_vid
from det3d.solver.fastai_optim import OptimWrapper
from det3d.torchie.trainer import DistSamplerSeedHook, Trainer, TrainerVID, obj_from_dict
...
def example_to_device(example, device=None, non_blocking=False) -> dict:
    ...
    for k, v in example.items():
        ...
        elif k in ["calib", "cam", "image_shape"]:
            calib = {}
...
def example_to_device_vid(example, device=None, non_blocking=False) -> dict:
    assert device is not None

    example_torch = {}
    float_names = ["voxels", "bev_map"]
    seq_len = len(example['num_points'])
    for k, v in example.items():
        if k in ["anchors", "anchors_mask", "reg_targets", "reg_weights", "labels", "hm",
            ...
            if k == "gt_boxes_and_cls":
                example_torch[k] = v.to(device, non_blocking=non_blocking)
            else:
                example_torch[k] = [v[seq].to(device, non_blocking=non_blocking) for seq in range(seq_len)]
        elif k == "calib":
            calib = {}
            for k1, v1 in v.items():
...
def batch_processor_vid(model, data, train_mode, **kwargs):
    ...
    # data = example_convert_to_torch(data, device=device)
    example = example_to_device_vid(data, device, non_blocking=False)
...
def train_detector_vid(model, dataset, cfg, distributed=False, validate=False, logger=None):
    if logger is None:
    ...
    if cfg.lr_config.type == "one_cycle":
        ...
        cfg.lr_config = None
    elif cfg.lr_config.type == "manual_stepping":
        optimizer = build_one_cycle_optimizer(model, cfg.optimizer)
        lr_scheduler = _create_learning_rate_scheduler(
            optimizer, cfg.lr_config, total_steps
        )
        cfg.lr_config = None
    elif cfg.lr_config.type == "exponential_decay":
        optimizer = build_one_cycle_optimizer(model, cfg.optimizer)
        lr_scheduler = _create_learning_rate_scheduler(
            optimizer, cfg.lr_config, total_steps
        )
    ...
    # import pdb; pdb.set_trace()
    trainer = TrainerVID(
        model, batch_processor_vid, optimizer, lr_scheduler, cfg.work_dir, cfg.log_level, cfg.max_save_num
    )
...
def train_detector(model, dataset, cfg, distributed=False, validate=False, logger=None):
    ...
    trainer = Trainer(
        model, batch_processor, optimizer, lr_scheduler, cfg.work_dir, cfg.log_level, cfg.max_save_num
    )
