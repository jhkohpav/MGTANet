...
class TrainerVID(object):
    ...
    def save_checkpoint(
        ...
        ckpt_list = glob.glob(str(out_dir+'/'+'epoch_*.pth'))
        ckpt_list.sort(key=os.path.getmtime)
        if ckpt_list.__len__() >= self.max_save_num:
            for cur_file_idx in range(0, len(ckpt_list) - self.max_save_num + 1):
