...
from det3d.core.sampler.sample_ops import DataBaseSamplerV2, DataBaseSampler_Temporal_sampling
...
def build_db_preprocess_temporal_sampling(db_prep_config, logger=None):
    logger = logging.getLogger("build_db_preprocess")
    cfg = db_prep_config
    if "filter_by_difficulty" in cfg:
        v = cfg["filter_by_difficulty"]
        return prep.DBFilterByDifficulty_temporal_filtering(v, logger=logger)
    elif "filter_by_min_num_points" in cfg:
        v = cfg["filter_by_min_num_points"]
        return prep.DBFilterByMinNumPoint_temporal_sampling(v, logger=logger)
    else:
        raise ValueError("unknown database prep type")
...
def build_dbsampler_vid(cfg, logger=None):
    logger = logging.getLogger("build_dbsampler_vid")
    prepors = [build_db_preprocess(c, logger=logger) for c in cfg.db_prep_steps]
    db_prepor = DataBasePreprocessor(prepors)
    rate = cfg.rate
    grot_range = cfg.global_random_rotation_range_per_object
...
def build_dbsampler_vid_temporal_sampling(cfg, logger=None):
    logger = logging.getLogger("build_dbsampler_vid with temporal sampling method")
    prepors = [build_db_preprocess_temporal_sampling(c, logger=logger) for c in cfg.db_prep_steps]
    db_prepor = DataBasePreprocessor(prepors)
    rate = cfg.rate
    grot_range = cfg.global_random_rotation_range_per_object
    groups = cfg.sample_groups
    ...
    db_infos_list = []
    info_path_list = cfg.db_info_path_list
    ...
    seq_len = cfg.seq_len
    sequence_mode = cfg.sequence_mode
    sampler = DataBaseSampler_Temporal_sampling(
        db_infos_list, groups, db_prepor, rate, grot_range, logger=logger,
        seq_len=seq_len, sequence_mode=sequence_mode
    )
